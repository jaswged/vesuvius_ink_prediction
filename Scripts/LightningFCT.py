...
class DS_out(nn.Module):
    ...
    def forward(self, x):
        ...
        x1 = self.conv2(x1)
        x1 = self.conv3(x1)
        out = torch.sigmoid(x1)  # Added myself from other implementation
...
class FCT(L.LightningModule):
    def __init__(self, CFG, in_channels = 6):
        super().__init__()
        ...
        att_heads = [2, 4, 8, 16, 8, 4, 2]
        # Number of channels in lower blocks
        # filters = [32, 64, 128, 256, 512, 256, 128, 64, 32]  # From original code
        # filters = [12, 24, 48, 96, 192, 96, 48, 24, 12]  # From my full implementation
        filters = [12, 24, 48, 96, 48, 24, 12]  # From my full implementation
        # filters = [8, 16, 32, 64, 128, 64, 32, 16, 8]  # Number of channels in lower blocks

        # number of blocks used in the model
        blocks = len(filters)
        print(f"Length of filters in this model: {blocks}")
        ...
        self.block_3 = Block_encoder_with_skip(filters[1], filters[2], att_heads[2], dpr[2], self.img_size // 4)
        # self.block_4 = Block_encoder_with_skip(filters[2], filters[3], att_heads[3], dpr[3], self.img_size // 8)
        self.block_5 = Block_encoder_without_skip(filters[2], filters[3], att_heads[3], dpr[3], self.img_size // 8)
        # self.block_6 = Block_decoder(filters[4], filters[5], att_heads[5], dpr[5], self.img_size // 32)
        self.block_7 = Block_decoder(filters[3], filters[4], att_heads[4], dpr[4], self.img_size // 16)
        ...
        self.ds7 = DS_out(in_channels=filters[4], out_channels=1, img_size=self.img_size // 8)  # outchannels were 4s
        self.ds8 = DS_out(in_channels=filters[5], out_channels=1, img_size=self.img_size // 4)
        self.ds9 = DS_out(in_channels=filters[6], out_channels=1, img_size=self.img_size // 2)
    ...
    def training_step(self, batch, batch_idx):
        x, y = batch
        pred_y = self(x)
        # pred_y = torch.argmax(pred_y, dim=0)  # Because I set the channels to 2?  was dim=1
        # pred_y = self.forward(x)

        # dsn
        down1 = F.interpolate(y, self.img_size // 2)
        down2 = F.interpolate(y, self.img_size // 4)
        # loss = f.binary_cross_entropy_with_logits(y_hat, y)  # From my other lightning
        ...
        y_pred_onehot = F.one_hot(y_pred, 1).permute(0, 3, 1, 2)  # Changed 4 -> 1
        dice = self.compute_dice(y_pred_onehot, y)
        # dice_LV = dice[3]
        # dice_RV = dice[1]
        # dice_MYO = dice[2]
        self.log('dice/all_train_dice', dice[1:].mean(), on_epoch=True)
        self.log('train_dice', dice[0].mean())
        my_dice = self.calc_dice_coef(y_pred_onehot, y)
        self.log('val_dice', my_dice)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # train dice calculations
        y_pred = torch.argmax(pred_y[2], axis=1)
        y_pred_onehot = F.one_hot(y_pred, 1).permute(0, 3, 1, 2)
        dice = self.compute_dice(y_pred_onehot, y)
        # dice_LV = dice[3]
        # dice_RV = dice[1]
        # dice_MYO = dice[2]
        ...
        my_dice = self.calc_dice_coef(y_pred_onehot, y)
        self.log('dice/my_val_dice', my_dice)
        ...
        return {
            ...
            "lr_scheduler": {
                "scheduler": torch.optim.lr_scheduler.ReduceLROnPlateau(
                    optimizer, mode='min', factor=self.CFG.lr_factor, min_lr=self.CFG.min_lr, patience=5),
    ...
    def compute_dice(self, pred_y, y):
        ...
        for c in range(num_classes):  # IndexError: index 1 is out of bounds for dimension 1 with size 1
        ...
        # intersection = torch.sum(pred_y[:, 0] * y[:, 0])
        # sum_masks = torch.sum(pred_y[:, 0]) + torch.sum(y[:, 0])
        # dice_scores[0] = (2. * intersection + epsilon) / (sum_masks + epsilon)
