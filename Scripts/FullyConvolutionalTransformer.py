"""
From this Github repo: https://github.com/Thanos-DB/FullyConvolutionalTransformer/blob/main/PyTorch/main.py
"""
import os
...
class Block_encoder_bottleneck(nn.Module):
    ...
    def forward(self, x, scale_img="none"):
        if ((self.blk=="first") or (self.blk=="bottleneck")):
            x1 = x.permute(0, 2, 3, 1)  # shape before torch.Size([B, 8, 224, 224]); after torch.Size([B, 224, 224, 8])
            ...
            x1 = self.layernorm(x1)
            x1 = x1.permute(0, 3, 1, 2)  # Breaks here on second pass
...
class FullyConvolutionalTransformer(nn.Module):
    def __init__(self, in_channels=6):
        ...
        # shape
        init_sizes = torch.ones((2, 224, 224, 1))  # Pass these in?
        init_sizes = init_sizes.permute(0, 3, 1, 2)  # torch.Size([2, 1, 224, 224])
    ...
    def forward(self, x):
        ...
        scale_img_3 = self.scale_img(scale_img_2)  # scale_img_3.shape torch.Size([40, 8, 56, 56])
        scale_img_4 = self.scale_img(scale_img_3)  # scale_img_4.shape torch.Size([40, 8, 28, 28])

        x = self.block_1(x)  # torch.Size([40, 8, 112, 112])
        # print(f"Block 1 out -> {list(x.size())}")
        skip1 = x
        x = self.block_2(x, scale_img_2)  # Breaks here!
...
if __name__ == "__main__":
    ...
    model = FullyConvolutionalTransformer().to(device)
