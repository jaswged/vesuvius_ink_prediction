...
class Transformer(nn.Module):
    ...
    def forward(self, x):
        x1 = self.attention_output(x)  # x.shape [1, 32, 112, 112]
        x1 = self.stochastic_depth(x1)
        x2 = self.conv1(x1) + x  # x2.shape [1, 32, 112, 112]
...
class Block_decoder(nn.Module):
    ...
    def forward(self, x, skip):
        ...
        x1 = self.layer2(x1)
        x1 = self.layer3(x1)
        out = self.trans(x1)  # breaks here on 9?!
        return out
...
class Block_encoder_without_skip(nn.Module):
    ...
    def forward(self, x):  # x.shape [2, 8, 224, 224]
        x = self.layernorm(x)  # x.shape [2, 8, 224, 224]
        x1 = self.layer1(x)  # x1.shape [2, 32, 224, 224]
        x1 = self.layer2(x1)  # x2.shape [2, 32, 112, 112]
        x1 = self.trans(x1)
        return x1
...
class Block_encoder_with_skip(nn.Module):
    ...
    def forward(self, x, scale_img):
        x1 = self.layernorm(x)
        x1 = torch.cat((self.layer1(scale_img), x1), axis=1)  # breaks here. x1 [1, 32, 112, 112], scaleimg [1, 8 , 112, 112]
        x1 = self.layer2(x1)
        x1 = self.layer3(x1)
        x1 = self.trans(x1)
        return x1
...
class FCT(nn.Module):
    def __init__(self, img_size, in_channels, out_channels, lr, lr_factor, min_lr):
        ...
        dpr = [x for x in np.linspace(0, stochastic_depth_rate, blocks)]  # [0.0, 0.0625, 0.125, 0.1875, 0.25, 0.3125, 0.375, 0.4375, 0.5]
    ...
    def forward(self, x):
        ...
        skip1 = x
        x = self.block_2(x, scale_img_2)  # torch.Size([1, 64, 56, 56])
    ...
    def training_step(self, batch, batch_idx):
        x, y = batch
        pred_y = self(x)  # returns 3 separate results at different image sizes?
        # dsn
        ...
        # train dice
        y_pred = torch.argmax(pred_y[2], axis=1)  # Presumably the full size image?
        ...
        return {
            ...
            "lr_scheduler": {
                "scheduler": torch.optim.lr_scheduler.ReduceLROnPlateau(
                    optimizer, mode='min', factor=self.lr_factor, min_lr=self.min_lr, patience=5),
